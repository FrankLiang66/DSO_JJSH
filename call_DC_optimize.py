# -*- coding: utf-8 -*-
"""
@File    : call_DC_optimize.py
@Author  : hm325800
@Time    : 2020/12/18 13:54

本文件用于实现JJSH_Form_DC_Optimize界面的功能，使用方法参见__main__中的调用方式
"""
...
from JJSH_Form_DC_Optimize import Ui_JJSH_Form_DC_Optimize
import functools


class JJSH_Form_DC_Optimize(QWidget, Ui_JJSH_Form_DC_Optimize):
    def __init__(self):
        super(JJSH_Form_DC_Optimize, self).__init__()
        self.setupUi(self)

        # 启动子主界面自己的定时器，并记录设计时的界面尺寸
        self.DC_timer = QtCore.QTimer()
        self.Dic_init_tagPosition = {}
        self.init_frame_DC_picture_width = self.frame_DC_picture.width()
        self.init_frame_DC_picture_height = self.frame_DC_picture.height()
    ...
    def btn_clicked_func(self, btn):
        if btn.objectName() == 'pushButton_DC_ctrl_on':
            self.pushButton_DC_ctrl_on.setEnabled(False)
            self.pushButton_DC_ctrl_off.setEnabled(True)
            self.DC_timer.start(1000)
            self.DC_timer.timeout.connect(self.DC_update_data)

        elif btn.objectName() == 'pushButton_DC_ctrl_off':
            self.pushButton_DC_ctrl_on.setEnabled(True)
            self.pushButton_DC_ctrl_off.setEnabled(False)
            self.DC_timer.stop()
    ...
    def DC_update_data(self):
        ...
        lims_data = pd.read_csv('./data/data_LIMS.csv', header=None)
    ...
    def get_frame_DC_picture_size(self):
        self.init_frame_DC_picture_width = self.frame_DC_picture.width()
        self.init_frame_DC_picture_height = self.frame_DC_picture.height()

        # 窗口tag自适应调整函数
    def adjust_tag_position(self):
        now_frame_DC_picture_width = self.frame_DC_picture.width()
        now_frame_DC_picture_height = self.frame_DC_picture.height()
        for key_name in self.Dic_init_tagPosition.keys():
            x_now = (now_frame_DC_picture_width / self.init_frame_DC_picture_width) \
                    * self.Dic_init_tagPosition[key_name].x()
            y_now = (now_frame_DC_picture_height / self.init_frame_DC_picture_height) \
                    * self.Dic_init_tagPosition[key_name].y()
            self.Dic_init_tagPosition[key_name] = QtCore.QRect(x_now,
                                                               y_now,
                                                               now_frame_DC_picture_width,
                                                               now_frame_DC_picture_height)
    # 事件函数
    def resizeEvent(self, event):

        now_width = self.frame_DC_picture.width()
        now_height = self.frame_DC_picture.height()

        move_width = now_width / self.init_frame_DC_picture_width
        move_height = now_height / self.init_frame_DC_picture_height
